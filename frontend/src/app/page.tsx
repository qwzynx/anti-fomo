    ...
    <div className="flex min-h-screen flex-col bg-background font-sans text-foreground">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-10">
        ...
        <section className="mb-10 grid grid-cols-1 items-start gap-4 md:grid-cols-2">
          <div className="animate-fade-up rounded-2xl border border-star-soft bg-surface p-5">
            <p className="mb-3 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-star">
            ...
            {loading ? (
              ...
              <button onClick={() => setSelected(topMatch)} className="group w-full text-left">
                <p className="font-bold leading-snug transition-colors group-hover:text-brand-soft-fg">
                  {topMatch.title}
                </p>
              ...
              <p className="text-sm text-muted">No strong match yet — check back soon.</p>
            )}
          </div>

          <div
            className="animate-fade-up rounded-2xl border border-line bg-surface p-5"
            style={{ animationDelay: "60ms" }}
          >
            <p className="mb-3 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-article">
            ...
            {loading ? (
                ...
                {trending.map((t, i) => (
                  <li key={i} className="text-sm leading-snug">
                    <button
                      ...
                      className="line-clamp-1 text-left font-semibold transition-colors hover:text-brand-soft-fg"
                    >
                      {t.title}
                    </button>
              ...
              <p className="text-sm text-muted">No stories right now.</p>
            )}
          </div>
        </section>
        ...
        <div className="mb-6 flex flex-wrap items-center gap-2">
          ...
          {filtersActive && (
            <button
              ...
              className="rounded-lg px-2 py-1 text-xs font-semibold text-brand-soft-fg hover:underline"
            >
              Reset
            </button>
          )}
