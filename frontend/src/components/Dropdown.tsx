  ...
  return (
    <div ref={rootRef} className={`relative ${className}`}>
      <button
        ...
        onKeyDown={onKeyDown}
        className={`flex w-full cursor-pointer items-center gap-2 rounded-xl border bg-surface py-2.5 pl-3.5 pr-3 text-sm font-medium text-foreground transition-colors ${
          open ? "border-brand" : "border-line hover:border-line"
        }`}
      >
      ...
      {open && (
        <ul
          ...
          role="listbox"
          aria-label={label}
          className={`animate-fade-up absolute z-30 max-h-64 w-full min-w-max overflow-y-auto rounded-xl border border-line bg-elevated p-1 ${
            ...
            return (
              <li
                ...
                className={`flex cursor-pointer items-center gap-2 rounded-lg px-3 py-1.5 text-sm transition-colors ${
                  i === activeIndex ? "bg-brand-soft text-brand-soft-fg" : "text-muted"
