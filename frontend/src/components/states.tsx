...
export function ErrorState({
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onRetry}
        className="mt-5 flex items-center gap-2 rounded-xl bg-brand px-4 py-2 text-sm font-semibold text-brand-fg transition-colors hover:bg-brand-hover"
      >
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center rounded-3xl border border-dashed border-line bg-surface px-6 py-16 text-center">
      <span className="flex h-12 w-12 items-center justify-center rounded-full bg-line-soft text-subtle">
        <InboxIcon className="h-6 w-6" />
      </span>
      <h3 className="mt-4 text-lg font-bold">{title}</h3>
      <p className="mt-1 max-w-md text-sm text-muted">{hint}</p>
      <button
        onClick={onAction}
        className="mt-5 rounded-xl border border-line px-4 py-2 text-sm font-semibold text-foreground transition-colors hover:bg-line-soft"
      >
        {actionLabel}
      </button>
    </div>
